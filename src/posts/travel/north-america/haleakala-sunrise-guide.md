---
title: "Haleakalā Sunrise: The Complete Guide to Maui's Most Unforgettable Morning"
description: "Everything you need to know to watch sunrise from the summit of Haleakalā — reservations, timing, what to wear, and why standing above the clouds at 10,000 feet is worth the 3am alarm."
date: 2026-05-21
...
<div class="pullquote">
<p>"We weren't watching the sunrise so much as watching the world below us wake up. The clouds caught the light before anything else did."</p>
</div>
...
<p>Book at <strong>recreation.gov</strong> — search "Haleakalā National Park sunrise." Reservations open 60 days in advance at midnight HST. The fee is $1 per reservation plus the park entrance fee ($30 per vehicle, valid for 3 days). Set a calendar reminder and be ready at your computer the moment they open.</p>
</div>
...
<p>If you're staying at the <a href="/posts/travel/hotel-reviews/andaz-maui-review/">Andaz Maui</a> or another resort in Wailea, ask the concierge about guided sunrise tours. They often have reserved slots that individual travelers can't access, and the guides add context about the geology and Hawaiian culture that makes the experience richer.</p>
</div>
</div>
...
<div class="info-box">
<div class="info-box-title">What to Pack in Your Car</div>
<p>Heavy jacket or fleece, long pants, warm socks, gloves, a beanie or hat, and a windproof layer if you have one. Bring a blanket if you have space — wrapping up while you wait for sunrise is not overkill. Hot coffee in a thermos is one of the best decisions you can make the night before.</p>
</div>
...
<div class="pullquote">
<p>"Standing above the clouds at 10,000 feet, watching the sun come up over the horizon while the entire island of Maui sat below us — there's nothing else like it."</p>
</div>
...
<div class="checklist-wrap" style="background:var(--cream);border:1px solid var(--border);border-radius:3px;padding:1.5rem;margin:2rem 0;">
  <div class="progress-text" id="progress-text-haleakala">0 of 10 items checked</div>
  <div class="progress-bar" style="background:#E8E0D4;border-radius:2px;height:4px;margin:.5rem 0 1.25rem;">
    <div class="progress-fill" id="progress-fill-haleakala" style="background:var(--gold);height:4px;border-radius:2px;width:0%;transition:width .3s;"></div>
  </div>
  <div id="checklist-haleakala">
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h1" onchange="updateHaleakalaProgress()">
      <label for="c-h1" style="font-size:14px;cursor:pointer;">Book reservation on recreation.gov (60 days in advance)</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h2" onchange="updateHaleakalaProgress()">
      <label for="c-h2" style="font-size:14px;cursor:pointer;">Look up exact sunrise time for your date</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h3" onchange="updateHaleakalaProgress()">
      <label for="c-h3" style="font-size:14px;cursor:pointer;">Fill up gas tank the night before</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h4" onchange="updateHaleakalaProgress()">
      <label for="c-h4" style="font-size:14px;cursor:pointer;">Lay out warm clothes the night before</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h5" onchange="updateHaleakalaProgress()">
      <label for="c-h5" style="font-size:14px;cursor:pointer;">Pack gloves, beanie, and a windproof layer</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h6" onchange="updateHaleakalaProgress()">
      <label for="c-h6" style="font-size:14px;cursor:pointer;">Prep hot coffee or tea in a thermos</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h7" onchange="updateHaleakalaProgress()">
      <label for="c-h7" style="font-size:14px;cursor:pointer;">Download offline map for the summit road</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h8" onchange="updateHaleakalaProgress()">
      <label for="c-h8" style="font-size:14px;cursor:pointer;">Set alarm for 3:00am</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;border-bottom:1px solid var(--border);">
      <input type="checkbox" id="c-h9" onchange="updateHaleakalaProgress()">
      <label for="c-h9" style="font-size:14px;cursor:pointer;">Arrive at summit at least 30 minutes before sunrise</label>
    </div>
    <div class="checklist-item" style="display:flex;align-items:center;gap:.75rem;padding:.5rem 0;">
      <input type="checkbox" id="c-h10" onchange="updateHaleakalaProgress()">
      <label for="c-h10" style="font-size:14px;cursor:pointer;">Plan a nap and beach afternoon for after</label>
    </div>
  </div>
</div>

<script>
function updateHaleakalaProgress() {
  var items = document.querySelectorAll('#checklist-haleakala input[type="checkbox"]');
  var checked = 0;
  ...
  document.getElementById('progress-text-haleakala').textContent = checked + ' of ' + total + ' items checked';
  document.getElementById('progress-fill-haleakala').style.width = pct + '%';
}
</script>
...
**Combine it with the Road to Hana.** If you're planning both — and you should — do Haleakalā first, earlier in your trip. The Road to Hana is a full-day commitment and you'll want to be fresh for it. We cover everything you need to know in our [Road to Hana guide](/posts/travel/north-america/road-to-hana-guide/).
...
*Planning your full Maui trip? Start with our [Complete Maui Travel Guide](/posts/travel/north-america/complete-maui-travel-guide/) for everything from where to stay to what to eat. And if you're using points to fund the trip, our [World of Hyatt guide](/posts/finance/credit-points/world-of-hyatt-program-explained/) covers how to make properties like the Andaz Maui nearly free.*
